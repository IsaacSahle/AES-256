...
public class AES{
	...
	public static char [] AES_Encrypt(char state [], char keyString []) {
		
		int numberOfRounds = 1;
		
		char key [] = KeyExpansion(keyString);
		//AddRoundKey(state, key);//whitening step
	...
	public static char[] KeyExpansion(char key []){
		...
		while(bytes < 240){
			...
			if(bytes % 32 == 0){
				...
				//wtf is this line...Stephs got it 
				//temp[0] ^= rcon(round_count);
	...
	public static void AddRoundKey(char[] state, char[] roundKey) {
		for(int i = 0; i < 16; i++) {
			state[i] ^= roundKey[i];
		}
	}
...
public static char [] init_encryption(char block [],char key []){
	return AES_Encrypt(block,key);
}

public static char [] init_decryption(char block [],char key []){
	//AES_Decrypt();
	return null;
}
