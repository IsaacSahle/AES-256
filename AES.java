import java.io.*;
import java.util.*;
import java.math.*;
import java.nio.file.*;
import java.nio.charset.Charset;

public class AES{

		public static final int in_sbox[] = {
	    };
		
		public static final int s_box[] =  {
		
		};
	
		public static final int mul2[] = {
		
		};
	
		public static final int mul3[] = {
		
		};
		
		public static final int rcon[] = {
		
		};
		public static final int mul9 [] = {
        };
		
		public static final int mul11 [] = {
		};

		public static final int mul13 [] = {
		};

		public static final int mul14 [] = {
		};

	
	/**
	* Takes the original 32 byte key and expands to a 240 byte key for all
	* 15 rounds of encryption/decryption
	* @param key the original key to be expanded
	* @return key the expanded 240 byte key
	*/	
    public static int [] KeyExpansion(int key []){
		int [] temp = new int[4];
		int bytes = 32;
		int roundCount = 1;
		int i = 0;

		while(bytes < 240){
			i = 0;
			while(i < 4){
				temp[i] = key[i + bytes - 4];
				i++;
			}

			//starting new round key generation
			if(bytes % 32 == 0){
				//rotate temporary array
				int temp_byte = temp[0];
				int count = 0;
				while(count < 3){
					temp[count] = temp[count + 1];
					count++;
				}

				temp[3] = temp_byte;
				count = 0;
				//s-box swap
				while(count < 4){
					temp[count] = s_box[temp[count]];					
					count++;
				}
				//XOR with rcon value	
				temp[0] = temp[0] ^ rcon[roundCount];
				roundCount++;
			}

			//extra s-box swap step for 256 bit encryption
			if(bytes % 32 == 16){
				int z = 0;
				while(z < 4){
					temp[z] = s_box[temp[z]];
					z++;
				}				
			}

			//generate next 4 bytes of key schedule
			int z = 0;
			while(z < 4){
				key[bytes] = key[bytes - 32] ^ temp[z];
				z++;
				bytes++;
			}			
		}
		return key;
    }
	
	/**
	* Creates a new 16 byte state by XORing the state with roundkey 
	* @param state the current block state 
	* @param roundKey the current round key  
	*/
	public static void AddRoundKey(int[] state, int[] roundKey) {
		for(int i = 0; i < 16; i++) {
			state[i] ^= roundKey[i];
		}
	}
	
	/**
	* Substitutes all 16 bytes in state with s-box values from the global lookup table 
	* @param state the current block state   
	*/
	public static void SubBytes(int [] state) {
		for (int i = 0; i < 16; i++) {
			state[i] = s_box[state[i]];
		}
	}
	
	/**
	* Substitutes all 16 bytes in state with the inverse s-box values from the global lookup table 
	* @param state the current block state   
	*/
	public static void Inv_SubBytes(int [] state) {
		for (int i = 0; i < 16; i++) {
			state[i] = in_sbox[state[i]];
		}
	}

	/**
	* Shifts the rows of state to the left 
	* @param state the current block state   
	*/
	public static void ShiftRows(int[] state) {
		int[] temp = new int[16];

		temp[0] = state[0];
		temp[1] = state[5];
		temp[2] = state[10];
		temp[3] = state[15];

		temp[4] = state[4];
		temp[5] = state[9];
		temp[6] = state[14];
		temp[7] = state[3];

		temp[8] = state[8];
		temp[9] = state[13];
		temp[10] = state[2];
		temp[11] = state[7];

		temp[12] = state[12];
		temp[13] = state[1];
		temp[14] = state[6];
		temp[15] = state[11];

		for (int i = 0; i < 16; i++) {
			state[i] = temp[i];
		}
	}
	
	/**
	* Shifts the rows of state to the right 
	* @param state the current block state   
	*/
	public static void Inv_ShiftRows(int[] state) {
		int[] temp = new int[16];

		temp[0] = state[0];
		temp[1] = state[13];
		temp[2] = state[10];
		temp[3] = state[7];

		temp[4] = state[4];
		temp[5] = state[1];
		temp[6] = state[14];
		temp[7] = state[11];

		temp[8] = state[8];
		temp[9] = state[5];
		temp[10] = state[2];
		temp[11] = state[15];

		temp[12] = state[12];
		temp[13] = state[9];
		temp[14] = state[6];
		temp[15] = state[3];

		for (int i = 0; i < 16; i++) {
			state[i] = temp[i];
		}
	}

	/**
	* Performs matrix multiplication with pre-computed values in table mul2 and mul3, to
	* mix up the columns of state    
	* @param state the current block state   
	*/
	public static void MixColumns(int[] state) {	
		
		int tmp[] = new int[16];
		
		tmp[0] = (mul2[state[0]] ^ mul3[state[1]] ^ state[2] ^ state[3]);
		tmp[1] = (state[0] ^ mul2[state[1]] ^ mul3[state[2]] ^ state[3]);
		tmp[2] = (state[0] ^ state[1] ^ mul2[state[2]] ^ mul3[state[3]]);
		tmp[3] = (mul3[state[0]] ^ state[1] ^ state[2] ^ mul2[state[3]]);

		tmp[4] = (mul2[state[4]] ^ mul3[state[5]] ^ state[6] ^ state[7]);
		tmp[5] = (state[4] ^ mul2[state[5]] ^ mul3[state[6]] ^ state[7]);
		tmp[6] = (state[4] ^ state[5] ^ mul2[state[6]] ^ mul3[state[7]]);
		tmp[7] = (mul3[state[4]] ^ state[5] ^ state[6] ^ mul2[state[7]]);

		tmp[8] = (mul2[state[8]] ^ mul3[state[9]] ^ state[10] ^ state[11]);
		tmp[9] = (state[8] ^ mul2[state[9]] ^ mul3[state[10]] ^ state[11]);
		tmp[10] = (state[8] ^ state[9] ^ mul2[state[10]] ^ mul3[state[11]]);
		tmp[11] = (mul3[state[8]] ^ state[9] ^ state[10] ^ mul2[state[11]]);

		tmp[12] = (mul2[state[12]] ^ mul3[state[13]] ^ state[14] ^ state[15]);
		tmp[13] = (state[12] ^ mul2[state[13]] ^ mul3[state[14]] ^ state[15]);
		tmp[14] = (state[12] ^ state[13] ^ mul2[state[14]] ^ mul3[state[15]]);
		tmp[15] = (mul3[state[12]] ^ state[13] ^ state[14] ^ mul2[state[15]]);
		
		for(int i = 0; i < 16; i++) {
			state[i] = tmp[i];
		}
		
	}

	/**
	* Performs matrix multiplication with pre-computed values in table mul9,mul11,mul13,mul14 to
	* mix up the columns of state    
	* @param state the current block state   
	*/
	public static void Inv_MixColumns(int[] state) {	
		
		int tmp[] = new int[16];
		
		tmp[0] = (mul14[state[0]] ^ mul11[state[1]] ^ mul13[state[2]] ^ mul9[state[3]]);
		tmp[1] = (mul9[state[0]] ^ mul14[state[1]] ^ mul11[state[2]] ^ mul13[state[3]]);
		tmp[2] = (mul13[state[0]] ^ mul9[state[1]] ^ mul14[state[2]] ^ mul11[state[3]]);
		tmp[3] = (mul11[state[0]] ^ mul13[state[1]] ^ mul9[state[2]] ^ mul14[state[3]]);

		tmp[4] = (mul14[state[4]] ^ mul11[state[5]] ^ mul13[state[6]] ^ mul9[state[7]]);
		tmp[5] = (mul9[state[4]] ^ mul14[state[5]] ^ mul11[state[6]] ^ mul13[state[7]]);
		tmp[6] = (mul13[state[4]] ^ mul9[state[5]] ^ mul14[state[6]] ^ mul11[state[7]]);
		tmp[7] = (mul11[state[4]] ^ mul13[state[5]] ^ mul9[state[6]] ^ mul14[state[7]]);

		tmp[8] = (mul14[state[8]] ^ mul11[state[9]] ^ mul13[state[10]] ^ mul9[state[11]]);
		tmp[9] = (mul9[state[8]] ^ mul14[state[9]] ^ mul11[state[10]] ^ mul13[state[11]]);
		tmp[10] = (mul13[state[8]] ^ mul9[state[9]] ^ mul14[state[10]] ^ mul11[state[11]]);
		tmp[11] = (mul11[state[8]] ^ mul13[state[9]] ^ mul9[state[10]] ^ mul14[state[11]]);

		tmp[12] = (mul14[state[12]] ^ mul11[state[13]] ^ mul13[state[14]] ^ mul9[state[15]]);
		tmp[13] = (mul9[state[12]] ^ mul14[state[13]] ^ mul11[state[14]] ^ mul13[state[15]]);
		tmp[14] = (mul13[state[12]] ^ mul9[state[13]] ^ mul14[state[14]] ^ mul11[state[15]]);
		tmp[15] = (mul11[state[12]] ^ mul13[state[13]] ^ mul9[state[14]] ^ mul14[state[15]]);

		for(int i = 0; i < 16; i++) {
			state[i] = tmp[i];
		}
		
	}


	/**
	* Validates that the block is a  valid line of 32 hex characters 
	* @param block the string to be validated   
	*/
	public static void validate_block(String block){
		if(block.length() != 32 || !(block.matches("^[0-9a-fA-F]*$"))){	
			System.out.println("ERROR: Not valid string: " + block);
			System.exit(-1);
		}
	}

	/**
	* Performs all 15 rounds of aes 256 decryption 
	* @param state the current block state
	* @param byteKey the original key to be used in decryption
	* @return state the decrypted message   
	*/
	public static int [] AES_Decrypt(int state [], int byteKey []){

		int numberOfRounds = 14;
		int key [] = KeyExpansion(byteKey);
		int from = key.length - 16;
		int to = key.length;
		int round_key [] = Arrays.copyOfRange(key,from,to);
		from -= 16;
		to -= 16;
 		
		AddRoundKey(state, round_key);//whitening step
		for (int i = numberOfRounds; i > 1; i--) {
			Inv_ShiftRows(state);
			Inv_SubBytes(state);
			int rounds [] = Arrays.copyOfRange(key,from,to);
			AddRoundKey(state, rounds);
			Inv_MixColumns(state);
			from -= 16;
			to -= 16;
		}
		
		//Final Round
		Inv_ShiftRows(state);
		Inv_SubBytes(state);
		round_key = Arrays.copyOfRange(key,from,to);
		AddRoundKey(state, round_key);
		return state;

	}

	/**
	* Performs all 15 rounds of aes 256 encryption 
	* @param state the current block state
	* @param byteKey the original key to be used in encryption
	* @return state the encrypted message   
	*/
	public static int [] AES_Encrypt(int state [], int byteKey []) {
			
			int numberOfRounds = 14;
			int key [] = KeyExpansion(byteKey);
			int from = 0;
			int to = 16;
			int round_key [] = Arrays.copyOfRange(key,from,to);
			from += 16;
			to += 16;
			AddRoundKey(state, round_key);

			for (int i = 1; i < numberOfRounds; i++) {
				SubBytes(state);
				ShiftRows(state);
				MixColumns(state);
				//specify round key
				int rounds [] = Arrays.copyOfRange(key,from,to);
				AddRoundKey(state, rounds);
				from += 16;
				to += 16;
			}
			
			//Final Round
			SubBytes(state);
			ShiftRows(state);
			round_key = Arrays.copyOfRange(key,from,to);
			AddRoundKey(state, round_key);
			return state;
	}

	public static void main(String[] args) {
		if(args.length != 3){
			System.out.println("ERROR: THREE PARAMATERS DUUUDE");
			System.exit(-1);
		}
		String encrypt = "e",decrypt = "d";
		String option = args[0],keyFile = args[1],inputFile = args[2];
		Scanner scan_keyFile;
		Scanner scan_InputFile;
		try{
			scan_keyFile = new Scanner(new File(keyFile));
			scan_InputFile = new Scanner(new File(inputFile));
		} catch(java.io.FileNotFoundException e){
			System.out.println("Unable to open " + keyFile + " or " + inputFile);
			return;
		}

		//converted key to binary
		String key = scan_keyFile.nextLine().trim();
		String binary = new BigInteger(key,16).toString(2);
		
		//pad binary string
		while(binary.length() < 256){
			binary = "0" + binary;
		}

		//key shedule in bytes
		int byteKey [] = new int [240];
		int count = 0;
		int index = 0;

		//insert first 32 bytes of key schedule, rest of schedule generated during key expansion 
		while(count < binary.length()){
			byteKey[index] = Integer.parseInt(binary.substring(count,count + 8),2);
			index++;
			count += 8;
		}

		ArrayList<String> list = new ArrayList<String>();

		//begin encryption/decryption: 16 byte blocks (128 bits)
		while(scan_InputFile.hasNextLine()){

			String block = scan_InputFile.nextLine().trim();
			validate_block(block);
			String temp = new BigInteger(block,16).toString(2);
			//pad line
			while(temp.length() < 128){
				temp = "0" + temp;
			}
			//create array containing all 16 bytes of the block
			int byteBlock [] = new int [16];
			index = 0;
			count = 0;
			while(count < temp.length()){
				byteBlock[index] = Integer.parseInt(temp.substring(count,count + 8),2);
				index++;
				count += 8;
			}

			int [] outputBlock;
			outputBlock = option.equalsIgnoreCase(encrypt)?AES_Encrypt(byteBlock,byteKey):AES_Decrypt(byteBlock,byteKey);			

			//convert output bytes to hex
			String outputLine = "";
			for (int i = 0;i < outputBlock.length;i++) {
				String hexChar = Integer.toHexString(outputBlock[i]);
				if(hexChar.length() == 1){
					hexChar = "0" + hexChar;
				}
				outputLine = outputLine + hexChar;
			}
			//store lines
			list.add(outputLine);
		}

		//write to file 
		Path outputPath;
		if(option.equalsIgnoreCase(encrypt)){
			outputPath = Paths.get(inputFile + ".enc");
		}else{
			outputPath = Paths.get(inputFile + ".dec");
		}
		
		try{
			Files.write(outputPath,list,Charset.forName("UTF-8"));
		}catch(java.io.IOException e){
			System.out.println("ERROR: Could not write to output file");
			System.exit(-1);
		}

	}


}
