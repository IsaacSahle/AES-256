...
public static final int in_sbox[] = {
	...
	public static int [] AES_Decrypt(int state [], int keyString []){

	int numberOfRounds = 14;
		int key [] = KeyExpansion(keyString);
	...
	public static int [] AES_Encrypt(int state [], int keyString []) {
		...
		System.out.println();*/
		//WARNING KEY ARRAY: SINGLE DIGITS ARE ONE BYTE 	
		AddRoundKey(state, round_key);//whitening step
		/*for (int i = 0;i < state.length;i++) {
			System.out.print(Integer.toHexString(state[i]));
		}
		...
		for (int i = 1; i < numberOfRounds; i++) {
			...
			/*for (int x = 0;x < state.length;x++) {
				System.out.print(Integer.toHexString(state[x]));
			}
				System.out.println();*/
    ...
    public static int [] KeyExpansion(int key []){
		...
		while(bytes < 240){
			...
			if(bytes % 32 == 0){
				...
				temp[0] = temp[0] ^ rcon[round_count];
				round_count++;
			}
			if(bytes % 32 == 16){	
				int z = 0;
...
public static int [] init_encryption(int block [],int key []){
	return AES_Encrypt(block,key);
}

public static int [] init_decryption(int block [],int key []){
	//AES_Decrypt();
...
public static void main(String[] args) {
	...
	//Vector<String> keyFile_inputVector = new Vector<String>();
	//Vector<String> inputFile_inputVector = new Vector<String>();
	...
	int bin_key [] = new int [240];
	int count = 0;
	int index = 0;
	while(count < binary.length()){
		bin_key[index] = Integer.parseInt(binary.substring(count,count + 8),2);
		index++;
		count += 8;
	}
	...
	while(scan_InputFile.hasNextLine()){
		...
		int bin_block [] = new int [16];
		index = 0;
		count = 0;
		while(count < temp.length()){
			bin_block[index] = Integer.parseInt(temp.substring(count,count + 8),2);
			index++;
			count += 8;
		}
		...
		if(option.equalsIgnoreCase(encrypt)){
			...
			for (int i = 0;i < output_block.length;i++) {
				System.out.print(Integer.toHexString(output_block[i]));
			}
		}else{
